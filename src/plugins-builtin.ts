import { tool } from 'ai';
import { z } from 'zod';
import { posix } from './ignore.js';
import type { Plugin } from './plugins.js';
import { exists, file, spawnCollect } from './fsx.js';
...
export const formatPlugin: Plugin = {
  ...
  afterTurn: async () => {
    for (const { file: manifestPath, script, command } of FORMATTERS) {
      if (!exists(manifestPath)) continue;

      if (script) {
        try {
          const pkg = file.json<{ scripts?: Record<string, string> }>(manifestPath);
      ...
      try {
        await spawnCollect(command, { timeoutMs: 60_000 });
      } catch {
