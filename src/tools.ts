...
import { jail, posix, walk } from './ignore.js';
import { GIT_TOOL_NAMES, gitTools } from './tools-git.js';
import { NET_TOOL_NAMES, netTools } from './tools-net.js';
import { exists, file, write, uuid, globToRegex, spawnCollect, statSync, rmSync, type ChildProcess } from "./fsx.js";
...
function isBinary(abs: string): boolean {
  const bytes = file.bytes(abs, 0, SNIFF_BYTES);
...
async function readNumbered(path: string, offset: number, limit: number): Promise<string> {
  const abs = jail(path);
  if (!exists(abs)) throw new Error(`No such file: ${path}`);
  if (isBinary(abs)) throw new Error(`${path} is a binary file, not text. Use bash if you need to inspect it.`);
  const lines = file.text(abs).split('\n');
...
export const applyPatchTool = tool({
  ...
  execute: async ({ patch }) => {
    ...
    for (const op of ops) {
      ...
      if (op.kind === 'delete') {
        if (!(exists(abs))) throw new Error(`cannot delete ${op.path}: no such file`);
        removals.push(abs);
      ...
      if (op.kind === 'add') {
        if (exists(abs)) throw new Error(`cannot add ${op.path}: it already exists`);
        writes.push({ abs, content: op.content.endsWith('\n') ? op.content : `${op.content}\n` });
        summary.push(`added ${op.path}`);
        continue;
      }

      if (!exists(abs)) throw new Error(`cannot update ${op.path}: no such file`);
      if (isBinary(abs)) throw new Error(`cannot update ${op.path}: it is a binary file`);

      const before = file.text(abs);
      ...
      if (op.moveTo) {
        ...
        if (exists(target)) throw new Error(`cannot move ${op.path}: ${op.moveTo} already exists`);
    ...
    for (const { abs, content } of writes) await write(abs, content);
    for (const abs of removals) rmSync(abs);
...
export const writeFileTool = tool({
  ...
  execute: async ({ path, content }) => {
    const abs = jail(path);
    const before = exists(abs) ? file.text(abs) : undefined;
...
export const globTool = tool({
  ...
  execute: async ({ pattern, limit = 200, includeIgnored = false }) => {
    const isMatch = globToRegex(pattern);
    const hits: string[] = [];
...
export const listDirTool = tool({
  ...
  execute: async ({ path = '.', depth = 2, includeIgnored = false }) => {
    ...
    for await (const rel of walk({ root, noIgnore: includeIgnored })) {
      ...
      if (parts.length <= depth) files.push({ rel, size: statSync(join(root, rel)).size });
...
async function grepWithRipgrep({ pattern, include, ignoreCase, includeIgnored }: GrepArgs): Promise<string | undefined> {
  ...
  let res;
  try {
    res = await spawnCollect(['rg', ...args], { cwd: process.cwd(), timeoutMs: 60_000 });
  } catch {
    return undefined;
  }
  const { stdout, stderr, exitCode: code } = res;
...
async function grepInJs({ pattern, include = '**/*', ignoreCase, includeIgnored }: GrepArgs): Promise<string> {
  ...
  const isMatch = globToRegex(include);
  const hits: string[] = [];
  for await (const rel of walk({ noIgnore: includeIgnored })) {
    if (!isMatch(rel)) continue;
...
type Running = { command: string; proc: ChildProcess; interrupted: boolean; killed?: Promise<unknown> };
 ...
 */
function killTree(proc: ChildProcess): Promise<unknown> {
  if (process.platform === 'win32' && proc.pid) {
    try {
      const taskkill = nodeSpawn('taskkill', ['/PID', String(proc.pid), '/T', '/F'], {
        stdio: 'ignore',
      });
...
export const bashTool = tool({
  ...
  execute: async ({ command, timeout = 120_000 }, { toolCallId, abortSignal }) => {
    ...
    const proc = nodeSpawn(shell[0]!, shell.slice(1), {
      cwd: process.cwd(),
      stdio: ['ignore', 'pipe', 'pipe'],
    });
    const timeoutTimer = setTimeout(() => proc.kill('SIGTERM'), timeout);
    if (abortSignal) abortSignal.addEventListener('abort', () => proc.kill('SIGTERM'));
    ...
    try {
      ...
      const [stdout, stderr, exitCode] = await Promise.all([
        pump(proc.stdout, toolCallId),
        pump(proc.stderr, toolCallId),
        exited,
      ]);
...
export const moveFileTool = tool({
  ...
  execute: async ({ from, to }) => {
    ...
    if (!exists(source)) throw new Error(`No such file: ${from}`);
    if (exists(target)) throw new Error(`${to} already exists. Delete it first or pick another name.`);

    write(target, file.bytes(source));
    rmSync(source);
